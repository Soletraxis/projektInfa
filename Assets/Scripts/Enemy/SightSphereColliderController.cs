...
public class SightSphereColliderController : MonoBehaviour {

    private void OnTriggerStay2D(Collider2D collision)
    {
        if (collision.CompareTag("Player") && !PlayerHiddenByObstacles())
        {
            GetComponentInParent<EnemyController>().playerInChaseRange = true;
            GetComponentInParent<EnemyController>().sightOfPlayerLost = false;
        }
    }
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        ...
        {
            GetComponentInParent<EnemyController>().playerInChaseRange = false;
            GetComponentInParent<EnemyController>().sightOfPlayerLost = true;
        }
    }
